...
def detect_anomaly(car_seqs, total_frames_num):
    ...
    for key in car_seqs.keys():
        ...
        pause_time = (stalling_car[-1, :] - stalling_car[0, :])[0] if stalling_car.shape[0] >= 2 else 0
        # Conditions
        if motion_ratio > POSITION_RATIO and speed_ratio > SPEED_RATIO and car.shape[0] > RECORD_THRESHOLD \
                and np.abs(x_center - 400) < 300 and pre_points[pre_points == 0].shape[0] < 1 \
                and pause_time > 110:
            print(key)
            anomalies[key] = car
...
if __name__ == '__main__':
    ...
    for file in files:
        ...
        for key in r.keys():
            ...
            anomaly_str += str(file.split('.')[0]) + ' ' + str(r[key] * video.duration*10) + ' 1' + '\n'
        # break
